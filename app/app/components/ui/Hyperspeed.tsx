...
// Module-level globals and helper classes to keep useEffect clean
...
class App {
  ...
  update(delta: number) {
    ...
    let time = this.clock.getDelta() + this.timeOffset;
...
const Hyperspeed = ({ effectOptions = DEFAULT_EFFECT_OPTIONS }: { effectOptions?: any }) => {
  ...
  useEffect(() => {
    ...
    console.error = (...args: any[]) => {
      if (args[0] && typeof args[0] === 'string' && args[0].includes('WebGLRenderer')) {
        return;
      }
  ...
  }, [effectOptions, hasWebGL]);

  if (hasWebGL === false) {
    return <CanvasHighway />;
  }
